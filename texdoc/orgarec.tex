...
\section{Technology}
The web application and data serving is done using Lift, which is also Scala based, enabling an easy integration with the core.
...
The final version of our project includes two different visualization types. The first one, which was the goal of the project, represents a force directed graph\footnote{https://github.com/mbostock/d3/wiki/Force-Layout} of courses which are linked by their relations. The second type of representation, which was found to be really useful to explore SHS courses, helps representing the repartition of students in courses given their section using a Sankey diagram\footnote{https://github.com/d3/d3-plugins/tree/master/sankey}. 

\subsection{Organic graph of courses}
This visualization is meant to show easily the relationship between courses and the study plan links. In this project we use only the percentage of students taking two courses to evaluate their distance, the more co-students they have, the closer they will be.  

\paragraph{Key principles}
In this visualizations courses are represented as nodes in a graph. Their relations are represented as links between nodes. 

Here are the parameters we represent and how we represent them: 
\begin{itemize}
\item Number of credits of a class: Size of node
\item Percentage of students following two classes: length and width of links
\end{itemize}

The settings tab enables to choose easily which study plans we want to represent using the following parameters: 
\begin{itemize}
\item Sections: the sections of courses we want to show
\item Semesters: the range of semesters in which we want to represent the data
\item Level: the level of courses (possible to select multiples)
\end{itemize}

It is also possible in the settings tab to modify the minimum number of co-students to represent an edge between two classes in order to generate a lighter or more complete graph. 

\paragraph{Displaying more information on courses}
When a course is selected (either by putting the mouse on the node or by using the search tool), some additional information (if available) is displayed on the lower left corner of the screen. For now, in this additional information field there is a graph showing the most related classes (by number of co-students) and the sections of students following this class. Course links' are also highlighted in red and the size of the course node is doubled to make it more visible.

These bar plots give a more precise information on a specific class than the original graph but would be useless if we used only them. 

\paragraph{Search tool}
On the upper right corner of the page, there is a search tool which enables to find easily a class, it uses autocomplete so finding a class is really easy. This tool was implemented because the user study conducted in the end of the semester showed that it was the most requested feature. 
...
The length of an edge is an approximation since it is often impossible to generate a graph with arbitrary lengths for all relations, however, the force directed layout gives a great approximation. 

\subsection{SHS explorer/Section explorer}
The section explorer considers the repartition of students from different sections in courses as a flow. The goal of this graph is to visualize easily which are the most popular courses depending on sections of students following them. 
